...
public abstract class Multiscope<E> extends JPanel
{
	...
	public void setRows(List<E> rows)
	{
		...
		for (Property<E> p : displayedProperties)
		{
			 ...
			 * plotter.getPlot().getSpace().getYDimension().setMin(p.
			 * getMinYDisplayed(), true); }
			 * 
			 * if (p.getMaxYDisplayed() != null) {
			 * plotter.getPlot().getSpace().getYDimension().setMax(p.
			 * getMaxYDisplayed(), true); }
			 */
			plotter.getPlot().getSpace().getXDimension().getLegend()
					.setText("time (" + p.getClock().getTimeUnit() + ")");
			plotter.getPlot().getSpace().getYDimension().getLegend()
					.setText(p.getHumanReadableNameAndUnit());
			plotter.getPlot().getSpace().getLegend().setText(
					plotter.getPlot().getSpace().getYDimension().getLegend().getText());
	...
	private Set<SwingPlotter> getPropertiesPlottersToShow()
	{
		...
		for (int columnindex : table.getSelectedColumns())
		{
			if (columnindex > 0)
			{
				...
				plotter.getPlot().getFigure().removeAllFigures();
				...
				for (int rowindex : table.getSelectedRows())
				{
					...
					propertyFigure.setColor(palette
							.getColor(plotter.getPlot().getFigure().getNbFigures()
									% palette.getNumberOfColors()));
					plotter.getPlot().getFigure().addFigure(propertyFigure);
				}
			}
		}
		...
		for (int columnindex : table.getSelectedColumns())
		{
			if (columnindex > 0)
			{
				...
				for (int r : table.getSelectedRows())
				{
					...
					plotter.getPlot().getFigure().addFigure(f);
				}
			}
		}
